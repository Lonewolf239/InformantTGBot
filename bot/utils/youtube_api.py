...
def detect_platform(url: str) -> str:
    ...
    if "instagram.com" in url_lower:
        return "Instagram"
    if "vk.com" in url_lower or "vk.video" in url_lower:
        return "VK"
...
def _sync_download(
    url: str, format_str: str, media_type: str
...
async def process_download_task(task_data: dict):
    ...
    try:
        ...
        await message.edit_text(
            format_styled_message(
                emoji="⏳", title=API_NAME, message=f"Скачивание файла... ({platform})"
            )
        )

        loop = asyncio.get_running_loop()
        media_data = await loop.run_in_executor(
            None, _sync_download, url, format_choice, media_type
        )
        ...
        if not media_data or not os.path.exists(media_data["file_path"]):
            await message.edit_text(
                format_styled_message(
                    emoji="❌",
                    title=API_NAME,
                    message=f"Не удалось получить доступ к медиа ({platform}).",
                )
            )
            return
        ...
        caption = format_styled_message(
            emoji=API_ICON,
            title=f"{API_NAME} ({platform})",
            message=f"<b>{media_data['title']}</b>",
        )
            ...
            if "reply" in err_msg or "not found" in err_msg:
                if media_type == "audio":
                    await message.chat.send_audio(
                        ...
                        performer=artist_name,
                        thumbnail=(
                            FSInputFile(thumbnail_path) if thumbnail_path else None
                        ),
                    )
                else:
        ...
        await message.edit_text(
            format_styled_message(
                emoji=API_ICON,
                title=API_NAME,
                message=f"✅ <b>{original_title} успешно загружено!</b>",
            )
        )
...
async def cmd_download_yt(message: types.Message):
    ...
    if len(args) < 2:
        ...
        await message.reply(
            format_styled_message(emoji=API_ICON, title=API_NAME, message=usage_text)
        )
        return
    ...
    for arg in params_args:
        if re.match(r"^-\d+$", arg):
            ...
            try:
                from_idx = int(arg.split("_")[1])
            except ValueError:
                pass
        elif arg.lower().startswith("-до_"):
            try:
                to_idx = int(arg.split("_")[1])
            except ValueError:
                pass
    ...
    status_msg = await message.reply(
        format_styled_message(
            emoji="⏳",
            title=API_NAME,
            message=f"Анализ ссылки ({platform}), подождите...",
        )
    )
    ...
    if not info:
        await status_msg.edit_text(
            format_styled_message(
                emoji="❌",
                title=API_NAME,
                message=f"Не удалось извлечь данные с {platform}.",
            )
        )
        return
    ...
    if "entries" in info or info.get("_type") == "playlist":
        ...
        if not entries:
            await status_msg.edit_text(
                format_styled_message(
                    emoji="❌",
                    title=API_NAME,
                    message="В данном плейлисте нет доступных треков.",
                )
            )
            return
        ...
        if not entries:
            await status_msg.edit_text(
                format_styled_message(
                    emoji="❌",
                    title=API_NAME,
                    message="По заданным критериям треков не найдено.",
                )
            )
            return
        ...
        await status_msg.edit_text(
            format_styled_message(
                emoji="⏳",
                title=API_NAME,
                message=f"Обнаружен плейлист ({len(entries)} треков).\nЗагрузка...",
            )
        )

        temp_playlist_dir = os.path.join(
            YT_DOWNLOAD_DIR, f"playlist_{uuid.uuid4().hex[:8]}"
        )
        ...
        for idx, entry in enumerate(entries, 1):
            ...
            await status_msg.edit_text(
                format_styled_message(
                    emoji="⏳",
                    title=API_NAME,
                    message=f"Скачивание: загрузка трека {idx}/{len(entries)}...",
                )
            )
            media_data = await loop.run_in_executor(
                None, _sync_download_playlist_item, entry_url, temp_playlist_dir
            )
        ...
        if not downloaded_files:
            ...
            await status_msg.edit_text(
                format_styled_message(emoji="❌", title=API_NAME, message=err_msg)
            )
        ...
        if len(downloaded_files) == 1:
            ...
            await status_msg.edit_text(
                format_styled_message(
                    emoji="⏳", title=API_NAME, message="Отправка трека в Telegram..."
                )
            )
            ...
            caption = format_styled_message(
                emoji="🎵",
                title=f"{API_NAME} ({platform})",
                message=f"<b>{os.path.basename(single_file)}</b>",
            )
            ...
            await status_msg.edit_text(
                format_styled_message(
                    emoji=API_ICON,
                    title=API_NAME,
                    message=f"✅ <b>Трек успешно отправлен!</b>{warning_msg}",
                )
            )
            shutil.rmtree(temp_playlist_dir, ignore_errors=True)
            return
        ...
        await status_msg.edit_text(
            format_styled_message(
                emoji="⏳", title=API_NAME, message="Упаковка: формируем ZIP-архивы..."
            )
        )
        ...
        for z_idx, group in enumerate(zip_groups, 1):
            await status_msg.edit_text(
                format_styled_message(
                    emoji="⏳",
                    title=API_NAME,
                    message=f"Отправка архива {z_idx}/{len(zip_groups)}...",
                )
            )

            is_multi = len(zip_groups) > 1
            caption_msg = (
                f"<b>{safe_playlist_title}</b> (Часть {z_idx}/{len(zip_groups)})"
                if is_multi
                else f"<b>{safe_playlist_title}</b>"
            )
            caption = format_styled_message(
                emoji="📦", title=f"{API_NAME} ({platform})", message=caption_msg
            )
            zip_name = (
                f"{safe_playlist_title}_part{z_idx}.zip"
                if is_multi
                else f"{safe_playlist_title}.zip"
            )
        ...
        await status_msg.edit_text(
            format_styled_message(
                emoji=API_ICON,
                title=API_NAME,
                message=f"✅ <b>Архивы успешно отправлены!</b>{warning_msg}",
            )
        )
        shutil.rmtree(temp_playlist_dir, ignore_errors=True)
        return
    ...
    video_formats = [
        f for f in formats if f.get("vcodec") != "none" and f.get("vcodec") is not None
    ]
    heights_dict = {}
    ...
    sorted_heights = sorted(
        [k for k in heights_dict.keys() if isinstance(k, int)], reverse=True
    ...
    for h in sorted_heights:
        ...
        format_str = (
            f"{v_info['format_id']}+{audio_format_id}"
            if is_video_only
            else v_info["format_id"]
        )
        ...
        warn = (
            f" ⚠️ >{YT_MAX_FILE_SIZE_MB}MB"
            if total_size > YT_MAX_FILE_SIZE_MB * 1024 * 1024
            else ""
        )
        ...
        inline_keyboard.append(
            [
                InlineKeyboardButton(
                    text=btn_text, callback_data=f"yt_dl|{choice_key}|{request_id}"
                )
            ]
        )
    ...
    choices["audio"] = {
        "size": audio_size,
        "format_str": audio_format_id,
        "type": "audio",
    }
    warn = (
        f" ⚠️ >{YT_MAX_FILE_SIZE_MB}MB"
        if audio_size > YT_MAX_FILE_SIZE_MB * 1024 * 1024
        else ""
    )
    inline_keyboard.append(
        [
            InlineKeyboardButton(
                text=f"🎵 Только Звук ({format_size(audio_size)}){warn}",
                callback_data=f"yt_dl|audio|{request_id}",
            )
        ]
    )
    ...
    if len(choices) == 1:
        ...
        inline_keyboard.insert(
            ...
            [
                InlineKeyboardButton(
                    text="🎬 Скачать (Лучшее качество)",
                    callback_data=f"yt_dl|best_auto|{request_id}",
                )
            ],
        )
    ...
    await status_msg.edit_text(
        format_styled_message(
            emoji=API_ICON,
            title=f"{API_NAME} ({platform})",
            message=f"<b>{info.get('title', 'Unknown Media')}</b>\n\nВыберите формат:",
        ),
...
async def process_yt_callback(callback: types.CallbackQuery):
    ...
    if not req_data:
        await callback.message.edit_text(
            format_styled_message(
                emoji="❌", title=API_NAME, message="Ссылка устарела. Повторите запрос."
            )
        )
        return
    ...
    if not choice_data:
        await callback.message.edit_text(
            format_styled_message(
                emoji="❌", title=API_NAME, message="Формат недоступен."
            )
        )
        return
    ...
    if choice_data["size"] > YT_MAX_FILE_SIZE_MB * 1024 * 1024:
        await callback.answer(
            text=f"❌ Файл ({choice_data['size'] / (1024 * 1024):.1f} MB) превышает лимит {YT_MAX_FILE_SIZE_MB} MB.",
            show_alert=True,
        )
        return
    ...
    try:
        ...
        await callback.answer(
            "❌ Очередь загрузок переполнена (макс. 50). Попробуйте позже.",
            show_alert=True,
        )
        return
