...
class Database:
    ...
    async def get_total_messages(self) -> int:
        async with aiosqlite.connect(self.db_path) as conn:
            async with conn.execute("SELECT total_messages FROM stats WHERE id = 1") as cursor:
    ...
    async def get_uptime_seconds(self) -> float:
        async with aiosqlite.connect(self.db_path) as conn:
            async with conn.execute("SELECT start_time FROM stats WHERE id = 1") as cursor:
                result = await cursor.fetchone()
                if result:
    ...
    async def update_user_message(self, user_id: int, username: str = None, user_link: str = None):
    ...
    async def log_rp_action(self, user_id: int, target_id: int, action: str, custom_text: str = None):
        async with aiosqlite.connect(self.db_path) as conn:
            await conn.execute('''
    ...
    async def clear_old_history(self, days: int = 30):
        ...
        async with aiosqlite.connect(self.db_path) as conn:
            await conn.execute("DELETE FROM command_history WHERE executed_at < ?", (cutoff,))
            await conn.execute("DELETE FROM auto_reply_history WHERE sent_at < ?", (cutoff,))
            await conn.execute("DELETE FROM rp_history WHERE executed_at < ?", (cutoff,))
            await conn.commit()
