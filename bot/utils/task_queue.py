...
class UniversalQueueManager:
    ...
    async def add_task(
        self, queue_name: str, func: Callable, *args, update_cb=None, **kwargs
        ...
        task = QueueTask(
            func=func, args=args, kwargs=kwargs, future=future, update_cb=update_cb
        )
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.exception(
                        f"Ошибка при выполнении задачи в очереди {queue_name}"
                    )
                    if not task.future.done():
