...
DISLIKED_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'disliked_memes.json')
...
def get_meme_keyboard(meme_url, user_id: int, is_rated=False):
    ...
    if not is_rated:
        buttons.append(InlineKeyboardButton(text="👎", callback_data=f"dislike_meme|{meme_id}"))

    return create_user_keyboard([buttons], user_id)


async def send_meme(target, is_callback=False, use_fallback=False, force_favorite=False):
    meme_data = None
...
async def cmd_meme(message: types.Message):
    ...
    success = await send_meme(message, is_callback=False, use_fallback=False, force_favorite=force_favorite)
    ...
    if not success:
        error_msg = format_styled_message(
            emoji="😔",
            title="Ошибка мемов",
            message="В избранном пока нет мемов!" if force_favorite else "API недоступен и избранное пусто.\nПопробуй позже!"
        )
...
async def add_favorite_callback(callback_query: types.CallbackQuery):
    ...
    if add_to_favorites(meme_to_save):
        ...
        try:
            new_keyboard = get_meme_keyboard(meme_url, callback_query.from_user.id, is_rated=True)
