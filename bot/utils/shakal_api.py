...
async def _process_shakal_video(
    vid1_path, vid2_path, concat_path, out_path, bot, vid1_msg, vid2_msg
):
    try:
...
async def cmd_shakal(message: types.Message):
    ...
    try:
        ...
        await status_msg.edit_text(
            format_styled_message(
                emoji="✅", title=API_NAME, message="Готово! Лови результат 👇"
            )
        )
        ...
        await message.reply_video(
            ...
            caption=format_styled_message(
                emoji=API_ICON, title=API_NAME, message="Идеальный датамош."
            ),
