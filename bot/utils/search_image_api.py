...
async def cmd_search_image(message: types.Message):
    ...
    wait_msg = await message.reply(
        format_styled_message(
            emoji="⏳",
            title=API_NAME,
            message=f"Ищу изображения по запросу: «<code>{query}</code>»...",
        )
    )
    ...
    if not urls:
        ...
        await wait_msg.edit_text(
            format_styled_message(
                emoji="❌",
                title=API_NAME,
                message="Ничего не найдено по вашему запросу. Попробуйте изменить формулировку.",
            )
        )
        return
    ...
    connector = aiohttp.TCPConnector(family=socket.AF_INET)
    async with aiohttp.ClientSession(
        connector=connector, headers=download_headers
    ) as session:
        for url in urls[:25]:
    ...
    if not downloaded_images:
        ...
        await wait_msg.edit_text(
            format_styled_message(
                emoji="❌",
                title=API_NAME,
                message="Не удалось загрузить найденные изображения. Попробуйте другой запрос.",
            )
        )
        return
    ...
    try:
        ...
        if len(downloaded_images) == 1:
            photo_file = BufferedInputFile(
                downloaded_images[0], filename="search_result.jpg"
            )
            ...
            for i, img_bytes in enumerate(downloaded_images):
                photo_file = BufferedInputFile(
                    img_bytes, filename=f"search_result_{i}.jpg"
                )
                if i == 0:
                    media_group.append(
                        InputMediaPhoto(media=photo_file, caption=caption)
                    )
                else:
        ...
        await wait_msg.edit_text(
            format_styled_message(
                emoji="✨",
                title=API_NAME,
                message=f"Успешно загружено и отправлено {len(downloaded_images)} шт. ниже! 👇",
            )
        )
        ...
        await wait_msg.edit_text(
            format_styled_message(
                emoji="❌",
                title=API_NAME,
                message="Ошибка отрисовки изображений. Попробуйте еще раз.",
            )
        )
