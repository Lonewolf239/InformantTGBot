    ...
    async def update_position(pos: int):
        try:
            ...
            await status_msg.edit_text(
                format_styled_message(emoji=icon, title=title, message=text)
            )
        except Exception:
            pass

    try:
        future, queue_position = await queue_manager.add_task(
            queue_name, func, *args, update_cb=update_position, **kwargs
        )
        ...
        error_msg = format_styled_message(
            emoji="❌", title=title, message="Произошла непредвиденная ошибка."
        )
