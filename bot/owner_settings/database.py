...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'owner_settings.db')
...
class OwnerSettingsDB:
    ...
    async def get_setting(self, key: str) -> bool:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT setting_value FROM owner_settings WHERE setting_key = ?', (key,)) as cursor:
    ...
    async def toggle_setting(self, key: str) -> bool:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute('UPDATE owner_settings SET setting_value = ? WHERE setting_key = ?', (new_val, key))
            await db.commit()
        return bool(new_val)
    ...
    async def get_all_settings(self) -> dict:
        settings = {}
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT setting_key, setting_value FROM owner_settings') as cursor:
